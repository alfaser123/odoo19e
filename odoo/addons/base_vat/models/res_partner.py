...
class ResPartner(models.Model):
    ...
    def _run_vat_checks(self, country, vat, partner_name='', validation='error'):
        _logger.info("base_vat   _run_vat_checks  country %s",country)
        """ OVERRIDE """
    ...
    def _check_vat_number(self, country_code, vat_number):
        ...
        _logger.warning("base_vat _check_vat_number country_code=%s", country_code)
        ...
        tst1 = getattr(self, check_func_name, None)
        tst2 = getattr(stdnum.util.get_cc_module(country_code, 'vat'), 'is_valid', None)
        _logger.warning("base_vat  _ check_vat_number   check_func_name=%s    check_func=%s   tst1=%s     tst2=%s  ", check_func_name,  check_func, tst1, tst2)
