...
class account_financial_reports_accounting_cxc_programas(models.Model):
    _name = "fpa.cxc.programas"
...
class account_financial_reports_accounting_cxc_programas_line(models.Model):
    _name = "fpa.cxc.programas.line"
    ...
    encabezado_id = fields.Many2one(
        'fpa.cxc.programas', string='Encabezado', ondelete='cascade')


class wizard_account_financial_reports_accounting_cxc_programas(models.TransientModel):
    _name = "fpa.cxc.programas.wizard"
    ...
    def generar(self):
        print 'GENERAR'
        self.env.cr.execute(''' DELETE FROM fpa_cxc_programas_line WHERE company_id = %s and user_id = %s''' % (
            self.company_id.id, self.env.user.id))
        print '0'
        self.env.cr.execute(''' DELETE FROM fpa_cxc_programas WHERE company_id = %s and user_id = %s''' % (
            self.company_id.id, self.env.user.id))
        print '1'
        where = ''
        ...
        self.env.cr.execute(''' INSERT INTO fpa_cxc_programas(date, date_to, company_id, user_id,chart_account_id,financial_id) VALUES ('%s','%s',%s,%s,%s,%s) RETURNING ID ''' % (
        ...
        #Movimientos
        movimientos = '''INSERT INTO fpa_cxc_programas_line (user_id,company_id,account_id, cuenta, valor, provision, encabezado_id,nivel, tipo_partner,rango)
                            SELECT %s,%s,aa.id, aa.code, sum(aml.debit - aml.credit) as valor, 0 as provision, %s::integer as encabezado_id, 100::integer nivel, td.name as tipo_partner,FV.name as rango
                                FROM account_move_line aml
                                    INNER JOIN account_move am ON am.id = aml.move_id
                                    INNER JOIN account_journal aj ON aj.id = am.journal_id AND aj.provision = False
                                    INNER JOIN account_account aa ON %s AND aa.type = 'receivable'
        ...
        provisiones = ''' UPDATE fpa_cxc_programas_line SET provision = prov.provision FROM (
                            SELECT aa.id, aml.partner_id, -1*sum(aml.debit) as provision
                                FROM account_move_line aml
                                    INNER JOIN account_move am ON am.id = aml.move_id
                                    INNER JOIN account_journal aj ON aj.id = am.journal_id AND aj.provision is True
                                    INNER JOIN account_account aa ON %s
                                    INNER JOIN account_account_type aat ON aat.id = aa.user_type AND aat.code = 'liability'
                                        WHERE aml.company_id = aa.company_id
                                        AND  aml.company_id = %s AND aa.parent_zero = %s
                                        AND aml.date <= '%s'
                                        AND am.state = 'posted'
                                        GROUP BY aa.id, aml.partner_id) as prov
                                WHERE fpa_cxc_programas_line.account_id = prov.id
                                ''' % (condition,self.env.user.company_id.id, self.chart_account_id.id, self.date_to)
        print provisiones
        self.env.cr.execute(provisiones)
        for structure in self.chart_account_id.structure_id.sorted(key=lambda r: r.sequence, reverse=True):
            # nivel
            nivel = '''INSERT INTO fpa_cxc_programas_line (user_id, company_id, account_id, cuenta, partner_id,
                        valor, provision ,encabezado_id,nivel)
                          select %s,%s,aar.id, aar.code, null, sum(valor), sum(provision),
                          %s, %s
                             from
                                 fpa_cxc_programas_line fbpl
        ...
        #resumen_account = '''INSERT INTO fpa_cxc_programas_line (user_id, company_id,account_id, partner_id, valor, provision, encabezado_id, nivel)
        #                     select %s,%s, account_id, null, sum(fbpl.valor),  sum(fbpl.provision) as provision, %s, 99
        #                         from fpa_cxc_programas_line fbpl
        ...
        chart_account = '''INSERT INTO fpa_cxc_programas_line (user_id,company_id,account_id, cuenta, partner_id, valor, provision ,encabezado_id,nivel)
                            select %s,%s,%s, '%s', null, sum(fbpl.valor) as valor, sum(fbpl.provision) as provision, %s, 98
                                from fpa_cxc_programas_line fbpl
        ...
        print '6'
        self._cr.execute(" DELETE FROM fpa_cxc_programas_line WHERE valor=0 and provision=0 ")
